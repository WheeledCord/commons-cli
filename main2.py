...
def draw_status_top_right(stdscr, w):
    """Removed status indicators (no WS/user/SETTINGS)."""
    return
...
def handle_main_key(key):
    ...
    if focus == FOCUS_HALLS:
        total_items = len(halls)
        if key == curses.KEY_DOWN:
            sel_hall_idx = (sel_hall_idx + 1) % total_items if total_items > 0 else 0
            ...
            # No +Join or +NEW actions
            pass

    elif focus == FOCUS_ROOMS:
        if current_rooms:
            total_items = len(current_rooms)
            if key == curses.KEY_DOWN:
